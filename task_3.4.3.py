...
class DataSet:
    ...
    def csv_filter_for_statistics(self):
        """Считывает вакансии из файла, содержащие все необходимые данные, очищает их от лишних пробелов и html тегов
    ...
    def get_statistics_using_pandas(self, path_to_csv_file: str, selected_vacancy: str, area_name: str):
        """Производит расчет статистики по требуемой профессии, используя Pandas.

        Args:
            path_to_csv_file (str): Путь до csv файла с вакансиями
            selected_vacancy (str): Профессия, по которой требуется получить статистику
            area_name (str): Регион, по которому требуется получить статистику
        """
        df = pd.read_csv(path_to_csv_file, delimiter=',')
        df["publish_year"] = df["published_at"].apply(lambda d: int(d[:4]))
        salary_by_year = df.groupby("publish_year")["salary"].mean()
        selected_vacancy_subset = df[(df["name"].str.contains(selected_vacancy)) & df["area_name"].str.match(area_name)]
        selected_vacancy_salary_by_year = selected_vacancy_subset.groupby("publish_year")["salary"].mean().dropna()
        ...
        self.fraction_by_area_sliced = fraction_by_area_sliced.to_dict()
        self.salary_by_area_sliced = salary_by_area_sliced.astype("int").to_dict()
        self.selected_vacancy_salary_by_year = selected_vacancy_salary_by_year.astype("int").to_dict()
        self.selected_vacancy_count_by_year = selected_vacancy_count_by_year.to_dict()
...
class InputConnect:
    ...
    def print_statistics(self, data_set: DataSet):
        ...
        print(f"Динамика уровня зарплат по годам для выбранной профессии и региона: {data_set.selected_vacancy_salary_by_year}")
        print(
            f"Динамика количества вакансий по годам для выбранной профессии и региона: {data_set.selected_vacancy_count_by_year}")
    ...
    def check_input(self):
        ...
        if self.output_type == "Вакансии":
            ...
            if self.csv_file_name and self.vacancy_name and self.area_name:
                return True
        return False
        ...
        if self.check_input():
            ...
            if not data_set.is_empty_file():
                if self.output_type == "Вакансии":
                    ...
                    data_set.get_statistics_using_pandas(self.csv_file_name, self.vacancy_name, self.area_name)
                    Report.generate_excel(data_set.salary_by_year, data_set.selected_vacancy_salary_by_year,
                                          ...
                                          self.vacancy_name, self.area_name)
...
class Report:
    ...
    def generate_excel(salary_by_year: dict, selected_vacancy_salary_by_year: dict,
                       ...
                       vacancy_name: str, area_name: str):
    ...
    def fill_year_sheet(sheet: Worksheet, salary_by_year: dict, selected_vacancy_salary_by_year: dict,
                        vacancies_count_by_year: dict,
                        selected_vacancy_count_by_year: dict, vacancy_name: str, area_name: str):
        ...
        sheet["C1"] = f"Средняя зарплата - {vacancy_name} по региону - {area_name}"
        sheet["C1"].font = Report.bold_font
        sheet["D1"] = "Количество вакансий"
        sheet["D1"].font = Report.bold_font
        sheet["E1"] = f"Количество вакансий - {vacancy_name} по региону - {area_name}"
    ...
    def generate_pdf(salary_by_year: dict, selected_vacancy_salary_by_year: dict, vacancies_count_by_year: dict,
        ...
        pdf_template = template.render(
            {"vacancy_name": vacancy_name, "area_name": area_name, "abs_path": abs_path_to_img, "salary_by_year": salary_by_year,
